...
export async function run() {
  ...
  const tx = await Mina.transaction(deployerAcc, () => {
    ...
    zkAppInstance.init(initialBalance, ownerAcc.toPublicKey(), potValue, Field(2), Field(100));
  });
  await tx.send().wait();
  ...
  console.log(
    'Owner starts the round. The range of odds is between: '+
    zkAppInstance.startRange.get().toString() + ' and ' +
    zkAppInstance.endRange.get().toString()
  );
  let guess = await askQuestion('what should be the chosen number? \n');
  try {
  ...
  try {
    ...
    console.log('What are the odds!! The guess is correct but ...');
  } catch {
